...
func tick():
	...
	if targetpos == Target.TargetType.LOST:
		return
...
func move_to(targetpos: Vector2):
	...
	var sign_ = 1
	var thrust_angle = ship.rotation.angle_to(thrust_vec)
	...
	if thrust_vec.length() > 0:
		...
		if thrust_angle < OFFSET_ALLOWED and thrust_angle > -OFFSET_ALLOWED:
			...
			ship.thrust(actual_thrust)
	
	self.state.x_thrust = x_thrust
	self.state.y_thrust = y_thrust

func brake(on_complete: Callable):
